<template>
  <div class="cartao" :class="{'galeria-vazia': object.images && object.images.length === 0}">
    <div class="informacoes">
      ...
      <span v-if="$store.state.user.profile === 'applicant' && object.institution">Localização:<b> {{ object.institution.fantasyName }}</b></span>
      <span v-if="$store.state.user.profile === 'applicant' && object.institution">Rua:<b> {{ object.institution.address.street }}<br/></b>Bairro:<b> {{ object.institution.address.neighborhood }}</b><br/>N°:<b> {{ object.institution.address.number }}</b></span>
    ...
    <div class="galeria">
      ...
      <button v-if="object.status === 1 && $store.state.user.profile === 'applicant' && isObserverButton && (isInterested || this.$router.currentRoute.name === 'Interesting')" class="button self-alignment defaultCursor is-warning" disabled ><i @click="alertObservando" class="fas fa-info-circle"></i>Interesse demonstrado</button>
